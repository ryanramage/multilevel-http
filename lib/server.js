...
module.exports = function (db, meta) {
  ...
  app.get('/package/:key', function (req, res, next) {

    res.writeHead(200, { 'Content-Type': 'application/javascript' })
    var name_ver = req.params['key'].split('@')

    if (!name_ver[1] || name_ver[1] === 'latest') {
      // latest we just get the highest sorted file
      var opts = getOpts(req.query)
      opts.start = 'file|' + name_ver[0] + '@|'
      opts.end = 'file|' + name_ver[0] + '@'
      opts.limit = 1
      opts.reverse = true
      db.valueStream(opts).on('data', function (data) {
        res.end(data)
      })

    } else {

      db.get('file|' + req.params['key'], req.query, function (err, value) {
  ...
  app.get('/data/:key', function (req, res, next) {
    if (req.query.valueEncoding === 'json' || req.query.encoding === 'json') {
      res.writeHead(200, { 'Content-Type': 'application/json' })
    }
    req.query.valueEncoding = 'binary'
